...
(function () {
  ...
  function openModal() {
    // Never open the modal while the welcome screen is showing. There is no
    // intake in progress to clear, and the overlay would trap the employee.
  ...
  function wire() {
    ...
    // Clicking the dimmed backdrop dismisses the modal so it can never trap anyone
  ...
  function init() {
    try {
      // Defensive: the modal must never be visible on first paint
      $('#modal').hidden = true;
      buildOptionGroups();
      wire();
    } catch (err) {
